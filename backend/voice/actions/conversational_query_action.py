...
class ConversationalQueryAction(BaseBriefingAction):
    ...
    async def run(self, action_input: BriefingActionInput, conversation_id: str) -> str:
        ...
        try:
            ...
            self._log_action("conversational_query", str(session_id), f"Query: {user_query}")
            ...
            async with get_database_session() as db_session:
                ...
                story_context = self._build_story_context(current_story, metadata, user_query)
    ...
    def _build_story_context(self, story, metadata: Optional[dict], user_query: str) -> str:
        """
        ...
        # Metadata context
        if metadata:
            context_parts.append(f"SOURCE: {metadata.get('newsletter_name', 'Unknown Newsletter')}")
