...
class AudioService:
    ...
    async def generate_story_audio(
        ...
        try:
            ...
            settings = VoiceSettings(
                stability=(
                    voice_settings.get("stability", 0.71) if voice_settings else 0.71
                ),
                similarity_boost=(
                    voice_settings.get("similarity_boost", 0.5)
                    if voice_settings
                    else 0.5
                ),
                style=voice_settings.get("style", 0.0) if voice_settings else 0.0,
                use_speaker_boost=(
                    voice_settings.get("use_speaker_boost", True)
                    if voice_settings
                    else True
                ),
            )
            ...
            # Generate audio
            logger.info(
                f"Generating audio for {len(story_text)} characters using voice {voice_id}"
            )
    ...
    async def stream_story_audio(
        ...
        try:
            ...
            logger.info(
                f"Streaming audio for {len(story_text)} characters using voice {voice_id}"
            )
    ...
    async def _check_rate_limit(self) -> None:
        ...
        async with self._request_lock:
            ...
            if self._requests_made >= self.rate_limit:
                ...
                if wait_time > 0:
                    logger.warning(
                        f"ElevenLabs rate limit exceeded, waiting {wait_time:.1f} seconds"
                    )
        ...
        try:
            ...
            for voice in voices.voices:
                self._voice_cache[voice.voice_id] = {
                    ...
                    "settings": {
                        "stability": (
                            getattr(voice.settings, "stability", 0.71)
                            if voice.settings
                            else 0.71
                        ),
                        "similarity_boost": (
                            getattr(voice.settings, "similarity_boost", 0.5)
                            if voice.settings
                            else 0.5
                        ),
                        "style": (
                            getattr(voice.settings, "style", 0.0)
                            if voice.settings
                            else 0.0
                        ),
                        "use_speaker_boost": (
