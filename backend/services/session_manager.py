...
from backend.models.database import ListeningSession, Story, Issue, User, Newsletter, UserSubscription
...
class BriefingSessionManager:
    ...
    async def create_session(
        ...
        logger.info(f"Created new session {session.id} for user {user_id} with {len(story_ids)} stories")
        return session
    
    async def get_current_story(self, session_id: UUID) -> Optional[Story]:
        """
        ...
        if not current_story:
            logger.error(f"Current story {session.current_story_id} not found for session {session_id}")
            return None
        
        return current_story
    
    async def advance_story(self, session_id: UUID) -> Optional[Story]:
        """
        ...
        if not next_story:
            logger.error(f"Next story {next_story_id} not found for session {session_id}")
            return None
        
        logger.info(f"Session {session_id} advanced to story {next_index + 1}/{len(session.story_order)}: {next_story.headline}")
        return next_story
    
    async def get_detailed_summary(self, session_id: UUID) -> Optional[str]:
        """
    ...
    async def get_story_metadata(self, session_id: UUID) -> Optional[dict]:
        """
        ...
        newsletter_query = select(Newsletter).where(Newsletter.id == issue.newsletter_id)
    ...
    async def get_session_progress(self, session_id: UUID) -> Optional[dict]:
        """
        ...
        return {
            ...
            "progress_percentage": round((current_index / total_stories) * 100, 1) if total_stories > 0 else 0,
            "session_status": session.session_status,
            "stories_remaining": total_stories - current_index - 1 if current_index < total_stories else 0
        }
    ...
    async def get_today_stories(self, user_id: UUID) -> List[Story]:
        ...
        query = select(Story).join(Issue).join(Newsletter).join(UserSubscription).where(
