...
class NewsletterParser:
    ...
    async def parse_newsletter_content(self, email_data: dict) -> dict:
        ...
        try:
            ...
            newsletter_info = {
                ...
                "raw_content": email_data.get("body_html", "")
            ...
            story_candidates = await self._extract_story_candidates(
                cleaned_content, soup
            )
            ...
            if len(valid_stories) > self.max_stories_per_newsletter:
                logger.info(
                    f"Limiting stories to {self.max_stories_per_newsletter} from {len(valid_stories)}"
                )
            ...
            logger.info(
                f"Successfully parsed newsletter with {len(newsletter_info['stories'])} stories"
            )
    ...
    async def _extract_story_candidates(
        self, cleaned_text: str, soup: BeautifulSoup
    ) -> list[str]:
        """
    ...
    async def _extract_by_patterns(self, text: str) -> list[str]:
        ...
        if not stories:
            ...
            if (
                current_story.strip()
                and len(current_story.strip()) > self.min_story_length
            ):
                stories.append(current_story.strip())
        ...
        try:
            ...
            for story in stories:
                if (
                    ...
                    and len(story.strip()) > self.min_story_length
                ):
    ...
    async def _create_story_summaries(self, story_text: str, index: int) -> dict | None:
        """
            ...
            content_sample = " ".join(
                [
                    ...
                    " ".join(
                        [
                            story["headline"]
                            for story in newsletter_info.get("stories", [])
                        ]
                    ),
                ]
            )
