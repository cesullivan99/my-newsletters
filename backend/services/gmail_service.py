...
class GmailService:
    ...
    async def exchange_code_for_tokens(self, authorization_code: str) -> dict[str, str]:
        ...
        try:
            ...
            logger.info(
                f"Successfully exchanged code for tokens for user: {token_info['user_email']}"
            )
            return token_info
    ...
    async def fetch_newsletter_emails(
        ...
        access_token: str,
        days_back: int = 7,
        newsletter_keywords: list[str] | None = None,
    ) -> list[dict]:
        """
        ...
        try:
            ...
            if newsletter_keywords:
                keyword_query = " OR ".join(
                    [f'"{keyword}"' for keyword in newsletter_keywords]
                )
    ...
    async def _parse_email_message(self, message: dict) -> dict | None:
        """
    ...
    async def _extract_email_body(self, payload: dict) -> dict[str, str] | None:
        """
    ...
    async def _is_newsletter_email(self, email_data: dict) -> bool:
        ...
        has_keywords = any(
            indicator in text_to_check for indicator in newsletter_indicators
        )
