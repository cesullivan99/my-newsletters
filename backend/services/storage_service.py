...
class StorageService:
    ...
    async def upload_audio_file(
        ...
        try:
            ...
            if len(audio_data) > self.max_file_size:
                raise ValueError(f"File size {len(audio_data)} exceeds maximum {self.max_file_size}")
            ...
            for attempt in range(self.max_retries):
                try:
                    ...
                    response = await self.supabase_client.storage.from_(self.bucket_name).upload(
                        path=file_path,
                        file=audio_data,
                        file_options={
    ...
    async def download_audio_file(self, file_path: str) -> bytes:
        ...
        try:
            ...
            response = await self.supabase_client.storage.from_(self.bucket_name).download(file_path)
    ...
    async def delete_audio_file(self, file_path: str) -> bool:
        ...
        try:
            ...
            response = await self.supabase_client.storage.from_(self.bucket_name).remove([file_path])
    ...
    async def cleanup_old_files(self, days_old: int = None) -> int:
        ...
        try:
            ...
            deleted_count = 0
            if files_to_delete:
                response = await self.supabase_client.storage.from_(self.bucket_name).remove(files_to_delete)
                if response:
    ...
    async def get_storage_usage(self, user_id: Optional[uuid.UUID] = None) -> Dict:
        """
    ...
    async def generate_presigned_url(
        ...
        try:
            ...
            response = await self.supabase_client.storage.from_(self.bucket_name).create_signed_url(
                path=file_path,
