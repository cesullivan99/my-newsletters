...
class BriefingRequest(BaseModel):
    ...
    voice_type: str | None = Field(
        default=None, description="Preferred voice type for TTS"
    )
    playback_speed: float | None = Field(
        default=1.0, description="Playback speed (0.5-2.0)"
    )
...
class SessionProgressResponse(BaseModel):
    ...
    session_status: str = Field(
        description="Status: 'playing', 'paused', or 'completed'"
    )
...
class StoryResponse(BaseModel):
    ...
    issue_date: datetime | None = Field(description="Publication date")
    summary_audio_url: str | None = Field(description="URL to summary audio file")
    full_text_audio_url: str | None = Field(description="URL to detailed audio file")
...
class SessionControlRequest(BaseModel):
    ...
    action: str = Field(
        description="Control action: 'pause', 'resume', 'skip', 'restart'"
    )
...
class VoiceActionInput(BaseModel):
    ...
    action_type: str = Field(
        description="Type of action: 'skip', 'tell_more', 'metadata'"
    )
...
class ActionResult(BaseModel):
    ...
    next_story_id: UUID | None = Field(
        default=None, description="ID of next story if advanced"
    )
    session_completed: bool = Field(
        default=False, description="True if session is now complete"
    )
...
class UserPreferences(BaseModel):
    ...
    default_voice_type: str = Field(
        default="default", description="Preferred voice type"
    )
    default_playback_speed: float = Field(
        default=1.0, description="Preferred playback speed"
    )
    summarization_depth: str = Field(
        default="high-level", description="Preferred detail level"
    )
...
class AudioStreamMessage(WebSocketMessage):
    ...
    audio_data: bytes | None = Field(default=None, description="Audio data chunk")
